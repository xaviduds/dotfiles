...
# Alacritty
[[ ! -d "$CONFIG_DIR/alacritty" ]] &&  rm -rf "$CONFIG_DIR/alacritty"
...
# Emacs
[ ! -d "$CONFIG_DIR/emacs" ]] && rm -rf "$CONFIG_DIR/emacs"
mkdir -p "$CONFIG_DIR/emacs"
cat "$DOTFILES_DIR/editor/emacs/init.el" > "$CONFIG_DIR/emacs/init.el"
cat "$DOTFILES_DIR/editor/emacs/config.org" > "$HOME_DIR/emacs/config.org"

# Ghostty
[ ! -d "$CONFIG_DIR/ghostty" ]] && rm -rf "$CONFIG_DIR/ghostty"
...
[ ! -d "$CONFIG_DIR/helix" ]] && mkdir -p "$CONFIG_DIR/helix/themes"
...
[ ! -d "$CONFIG_DIR/nushell" ]] && mkdir -p "$CONFIG_DIR/nushell"
...
[ ! -d "$CONFIG_DIR/waybar" ]] && mkdir -p "$CONFIG_DIR/waybar"
